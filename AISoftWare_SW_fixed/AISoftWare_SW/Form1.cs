...
namespace AISoftWare_SW
{
    public partial class Form1 : Form
    {
        ...
        {
            NeedCookie needCookie = new NeedCookie();

            INILoader kLoader = new INILoader();
            kLoader.SetFileName("AISoftWare_SW.ini");
            kLoader.SetTitle("NeedCookie");

            needCookie.szCfbm = kLoader.LoadString("szCfbm", "");
            needCookie.szSecureNextAuth = kLoader.LoadString("m_szSecureNextAuth", "");
            needCookie.szCfClearance = kLoader.LoadString("m_szCfClearance", "");

            SeleniumHelper.OnSetCookie(needCookie);
